...
namespace BankWebbApp.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var countries = _context.Accounts.Where(x=>x.Balance>0).Include(x=>x.Dispositions).ThenInclude(x=>x.Customer).SelectMany(x => x.Dispositions).Select(x=>x.Customer.Country).Distinct().ToList();
